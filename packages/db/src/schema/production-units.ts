...
export const productionUnitStatusEnum = pgEnum(
...
export const productionUnits = pgTable(
  "production_units",
  {
    ...
    qcActorUserId: bigint("qc_actor_user_id", { mode: "number" }).references(
      () => users.id,
    ),
  ...
  (t) => ({
    ...
    batchVerdictIdx: index("production_units_batch_verdict_idx").on(
      t.batchId,
      t.qcVerdict,
    ),
  }),
